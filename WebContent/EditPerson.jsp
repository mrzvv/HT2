         ...
         for(Map.Entry<String, String> phone : person.getPhones().entrySet())
           {
            %>
				...
				<a href="<%=request.getContextPath()%>/?action=editPhone&amp;phoneId=<%=phone.getKey()%>&amp;id=<%=person.getId()%>">Редактировать</a>
             	<a href="<%=request.getContextPath()%>/?action=deletePhone&phoneId=<%=phone.getKey()%>&id=<%=person.getId()%>">Удалить</a><br/>
